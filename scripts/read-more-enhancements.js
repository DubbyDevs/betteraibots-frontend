...
function getToolName(article, app) {
  if (app && app.name) return cleanText(app.name);
...
function getFeatureBullets(app, max = 5) {
  if (!app || !Array.isArray(app.features)) return [];
  return app.features
    .map(cleanText)
    .filter(Boolean)
    .slice(0, max);
...
function buildReadMoreEnhancement(article, app) {
  if (!article) return { markdown: '', faq: [] };

  const sourceContent = String(article.content || '');
  const name = getToolName(article, app);
  const category = cleanText((app && app.category) || 'AI tools');
  const description = cleanText((app && app.description) || article.preview || article.title || '');
...
function buildReadMoreEnhancementMarkdown(article, app) {
  return buildReadMoreEnhancement(article, app).markdown;
}

function buildReadMoreFaqSchema(article, app) {
